...
class AOTv2Engine(nn.Module):
    ...
    def update_short_term_memory(self, curr_mask, curr_id_emb=None):
        ...
        for layer_idx in range(len(lstt_curr_memories)):
            ...
            curr_v = self.AOT.LSTT.layer.linear_V(curr_v + curr_id_emb)
    ...
    def generate_loss_mask(self, gt_mask, step, return_prob=False):
        ...
        curr_lstt_embs = self.curr_lstt_output[0]
        num_lstt_layer = len(curr_lstt_embs)

        all_lstt_loss = 0
        all_pred_lstt_mask = []
        all_pred_lstt_prob = []
        for idx in range(num_lstt_layer):
            self.curr_lstt_output[0] = curr_lstt_embs[:(idx + 1)]

            self.decode_current_logits()
            loss = self.calculate_current_loss(gt_mask, step)
            all_lstt_loss = all_lstt_loss + loss

            if return_prob:
                mask, prob = self.predict_current_mask(return_prob=True)
                all_pred_lstt_prob.append(prob)
            else:
                mask = self.predict_current_mask()
            all_pred_lstt_mask.append(mask)

        all_lstt_loss = all_lstt_loss / num_lstt_layer
        self.all_pred_lstt_mask = all_pred_lstt_mask
        self.all_pred_lstt_prob = all_pred_lstt_prob

        if return_prob:
            return all_lstt_loss, mask, prob
        else:
            return all_lstt_loss, mask
...
class AOTv2InferEngine(nn.Module):
    def __init__(self,
                 aot_model,
                 gpu_id=0,
                 long_term_mem_gap=9999,
    ...
    def add_reference_frame(self, img, mask, obj_nums, frame_step=-1):
        ...
        while (aot_num > len(self.aot_engines)):
            new_engine = AOTv2Engine(self.AOT, self.gpu_id,
