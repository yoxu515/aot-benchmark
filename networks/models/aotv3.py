...
class AOTv3(nn.Module):
    def __init__(self, cfg, encoder='mobilenetv2', decoder='fpn'):
        ...
        for i,s in enumerate(scales):
            ...
            if cfg.MODEL_USE_ID_BANK_POST_CONV:
                norm = LayerNorm2d if cfg.MODEL_ID_BANK_POST_CONV_USE_LN else nn.BatchNorm2d
                self.id_post_convs.append(BasicBlock(cfg.MODEL_MS_ENCODER_EMBEDDING_DIMS[i],
                                            cfg.MODEL_MS_ENCODER_EMBEDDING_DIMS[i],BatchNorm=norm))
    ...
    def get_id_embs(self,x):
        if self.cfg.MODEL_USE_ID_ENCODER:
            id_features = self.id_encoder(x)[::-1]
            id_embs = []
            for i in range(len(id_features)):
                id_emb = self.id_encoder_adaptors[i](id_features[i])
                # id_emb = self.id_norms[i](id_emb.permute(2, 3, 0, 1)).permute(2, 3, 0, 1)
                id_embs.append(id_emb)
        else:
            id_embs = []
            for i in range(len(self.patch_wise_id_banks)):
                id_emb = self.patch_wise_id_banks[i](x)
                id_emb = self.id_norms[i](id_emb.permute(2, 3, 0, 1)).permute(2, 3, 0, 1)
